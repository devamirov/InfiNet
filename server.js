    ...
    try {
        ...
        if (fileData) {
            try {
                ...
                console.log(`[File Upload] Base64 data length: ${fileData.length} characters`);
                
                // Decode base64 and write to file
                const fileBuffer = Buffer.from(fileData, 'base64');
                console.log(`[File Upload] Decoded buffer size: ${fileBuffer.length} bytes`);
                ...
                fileUrl = `/uploads/files/${sanitizedFileName}`;
        ...
        console.log(`[File Upload] Inserting into database: fileId=${fileId}, projectId=${projectId}, fileName=${fileName}, fileUrl=${fileUrl}`);
        
        // Insert file into database using Promise wrapper
        await new Promise((resolve, reject) => {
            db.run(`
                INSERT INTO project_files (id, projectId, name, url, uploadedAt)
                VALUES (?, ?, ?, ?, datetime('now'))
            `, [fileId, projectId, fileName, fileUrl], function(err) {
                if (err) {
...
app.post('/api/invoices', authenticateToken, async (req, res) => {
    try {
        ...
        if (receiptData) {
            try {
                // Determine file extension (default to .pdf for receipts)
                const fileExt = receiptData.startsWith('/9j/') || receiptData.startsWith('iVBOR') ? '.jpg' : '.pdf';
                ...
                console.log(`Receipt saved to: ${filePath} (${fileBuffer.length} bytes)`);
            } catch (fileError) {
        ...
        `, [invoiceId, projectId, title, parseFloat(amount), currency || 'USD', issuedDate, dueDate, receiptUrl], function(err) {
            if (err) {
            ...
            `, [
                ...
                `${invoiceUser?.name || invoiceUser?.email || 'User'}: ${title} - $${amount}`,
                null, // No userId - this is for admin
                projectId,
                invoiceId,
                'invoices',
...
app.get('/api/auth/profile', authenticateToken, async (req, res) => {
    try {
        ...
        // Fetch avatar from user_preferences (prioritize over users.avatar for cross-device sync)
        db.get('SELECT avatar FROM user_preferences WHERE userId = ?', [req.user.id], (err, prefsRow) => {
            if (err) {
                console.error('Error fetching user preferences:', err);
                // Continue with user.avatar if preferences fetch fails
            }
            
            // Use avatar from user_preferences if available, otherwise fallback to users.avatar
            const avatar = (prefsRow && prefsRow.avatar) ? prefsRow.avatar : user.avatar;
