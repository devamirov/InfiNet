...
const GREETING_MESSAGE = "Hi! I'm your InfiNet AI Assistant — ask me anything.";
...
function initializeWhatsApp() {
  ...
  whatsappClient.on('ready', async () => {
    ...
    try {
      ...
      console.log('✅ WhatsApp display name set to "InfiNet AI"');
    } catch (error) {
      console.warn('⚠️ Could not set display name (may require manual setup):', error.message);
    }
  });
  ...
  whatsappClient.on('message', async (message) => {
    try {
      ...
      console.log('📨 WhatsApp message routing:', {
        hasVoice,
        hasImage,
        isImageRequest,
...
async function handleWhatsAppVoiceMessage(message, from) {
  try {
    ...
    console.log('🎤 WhatsApp voice message received:', {
    ...
    try {
      ...
      console.log('✅ WhatsApp: Gemini native audio response received:', aiResponseText.substring(0, 50));
      ...
      try {
        if (openai) {
          ...
          if (whisperResult && whisperResult.text && whisperResult.text.trim()) {
            ...
            console.log('🎤 Gemini native audio: Backup transcription for image check:', checkTranscribed.substring(0, 50));
            
            // Check if the transcribed text is an image generation request
            if (isImageGenerationRequest(checkTranscribed)) {
              console.log('🎨 Image generation request detected via backup transcription');
        ...
        console.log('ℹ️ Backup transcription failed (non-critical):', transcriptionError.message);
      }

    } catch (geminiError) {
      if (geminiError.message === 'FALLBACK_TO_WHISPER_GROQ') {
        // Fallback: Use Whisper to transcribe, then Groq
        console.log('🔄 WhatsApp: Using Whisper transcription + Groq fallback...');
        ...
        try {
          ...
          console.log('✅ WhatsApp: Audio transcribed with Whisper:', transcribedText ? transcribedText.substring(0, 50) : '(empty)');
          ...
          if (isImageGenerationRequest(transcribedText)) {
            console.log('🎨 Image generation request detected in voice message (before AI processing)');
          ...
          const detectMessageLanguage = (text) => {
            ...
            console.log('🌐 WhatsApp Voice Language detection:', { hasArabic, transcribedPreview: text.substring(0, 50) });
          ...
          console.log('✅ WhatsApp: Groq fallback response received (via Whisper):', aiResponseText.substring(0, 50));

        } catch (whisperError) {
          console.error('❌ WhatsApp: Whisper transcription error:', whisperError);
    ...
    if (!textToCheck && usedNativeAudio && aiResponseText) {
      ...
      console.log('🎤 Gemini native audio: Checking AI response for image generation intent');
    }
    ...
    if (isImageRequest) {
      ...
      console.log('🎨 Image generation request detected in voice message (final check)');
    ...
    console.log(`🌐 WhatsApp Voice: Detected ${responseLanguage} response, using voice: ${selectedVoice}`);
...
async function handleWhatsAppChat(message, userMessage, from) {
  try {
    ...
    if (isImageRequest) {
      console.log('🎨 Fallback: Image generation request detected in handleWhatsAppChat - routing to image generation');
    ...
    let formattedResponse = aiResponse
      ...
      .replace(/\*\s*،/g, '،')                      // Remove asterisks before Arabic comma
      .replace(/\*\s*。/g, '。')                     // Remove asterisks before full stops
      // Remove patterns like "text**." or "text*." (asterisks at end of sentences)
      .replace(/\*\*\./g, '.')                      // Remove ** before period
      .replace(/\*\./g, '.')                        // Remove * before period
      .replace(/\*\*،/g, '،')                       // Remove ** before Arabic comma
      .replace(/\*،/g, '،')                         // Remove * before Arabic comma
    ...
    if (isQuotaError(error)) {
      console.log('⚠️ Quota exceeded on all API keys for WhatsApp chat');
...
async function handleWhatsAppImageToImage(message, prompt, from) {
  try {
    ...
    if (!rateLimitCheck.allowed) {
      console.log(`🚫 WhatsApp rate limit exceeded for user: ${from}, type: replicate`);
      await message.reply(`🚫 ${rateLimitCheck.message}`);
      return;
    }

    await message.reply('🖼️ Processing your image... This may take a moment.');
    ...
    console.log('🖼️ WhatsApp image-to-image request (Replicate nano-banana):', { 
      hasImage: true, 
    ...
    console.log('🔄 Calling Replicate google/nano-banana for WhatsApp image-to-image...');
    ...
    try {
      ...
      await message.reply(media, undefined, { caption: `✨ Transformed: ${finalPrompt}` });
      
      // Increment rate limit after successful generation
      // Note: WhatsApp uses phone number as identifier, IP tracking not available for WhatsApp messages
      incrementRateLimit(userId, 'replicate', null);
      console.log(`✅ WhatsApp image-to-image successful, rate limit incremented for: ${from}`);
...
async function handleWhatsAppImageGeneration(message, prompt, from) {
  try {
    ...
    if (!rateLimitCheck.allowed) {
      console.log(`🚫 WhatsApp rate limit exceeded for user: ${from}, type: dalle`);
      await message.reply(`🚫 ${rateLimitCheck.message}`);
      return;
    }

    await message.reply('🎨 Generating your image... This may take a moment.');
    ...
    console.log('🎨 Image generation - Original prompt:', originalPrompt.substring(0, 150));
    ...
    if (hasNonEnglish) {
      ...
      if (!cleanedPrompt || cleanedPrompt.length < 3) {
        ...
        console.log('⚠️ Non-English: Cleaned prompt too short, using original');
      }
    } else {
      ...
      if (!cleanedPrompt || cleanedPrompt.length < 3) {
        cleanedPrompt = originalPrompt;
        console.log('⚠️ English: Cleaned prompt too short, using original');
      }
    }
    
    console.log('🎨 Final prompt to DALL-E:', cleanedPrompt.substring(0, 150));
    ...
    if (!openai) {
      console.error('❌ OpenAI not configured - OPENAI_API_KEY missing');
      await message.reply('Image generation service is not configured. Please contact support.');
      return;
    }

    console.log('🎨 Calling DALL-E 3 with prompt:', cleanedPrompt.substring(0, 100));
    ...
    try {
      ...
      console.log('✅ DALL-E 3 response received:', response.data?.[0]?.url ? 'Image URL generated' : 'No image URL in response');
      ...
      await message.reply(media, undefined, { caption: `🎨 Generated: ${cleanedPrompt}` });
      console.log('✅ Image sent successfully via WhatsApp');
      
      // Increment rate limit after successful generation
      // Note: WhatsApp uses phone number as identifier, IP tracking not available for WhatsApp messages
      incrementRateLimit(userId, 'dalle', null);
      console.log(`✅ WhatsApp image generation successful, rate limit incremented for: ${from}`);
