...
/**
 * List of common valid TLDs (Top Level Domains)
 * This is not exhaustive but covers the most common ones
 */
const COMMON_TLDS = [
  'com', 'org', 'net', 'edu', 'gov', 'mil', 'int',
  'io', 'co', 'ai', 'app', 'dev', 'tech', 'online', 'site', 'website',
  'xyz', 'info', 'biz', 'name', 'pro', 'tv', 'me', 'cc', 'ws',
  'uk', 'us', 'ca', 'au', 'de', 'fr', 'it', 'es', 'nl', 'be', 'ch', 'at', 'se', 'no', 'dk', 'fi', 'pl', 'cz', 'ie', 'pt', 'gr',
  'jp', 'cn', 'in', 'kr', 'sg', 'hk', 'tw', 'my', 'th', 'ph', 'id', 'vn', 'nz',
  'br', 'mx', 'ar', 'cl', 'co', 'pe', 'za', 'ae', 'sa', 'eg', 'ma', 'ng', 'ke',
  'services', 'cloud', 'digital', 'global', 'world', 'space', 'store', 'shop', 'blog', 'news', 'media', 'email', 'host', 'design', 'agency', 'marketing', 'consulting', 'solutions', 'systems', 'network', 'technology', 'software', 'support', 'tools', 'center', 'group', 'company', 'international', 'business', 'enterprise', 'corp', 'inc', 'ltd', 'llc'
];
...
function validateDomain(domain) {
  ...
  // Validate TLD length (must be 2-63 characters)
  if (tld.length < 2 || tld.length > 63) {
    return { 
      valid: false, 
      error: 'Invalid domain. The domain extension (TLD) must be between 2 and 63 characters.' 
    };
  }
  ...
  if (!/^[a-z]+$/.test(tld)) {
    return { 
      valid: false, 
      error: 'Invalid domain. The domain extension (TLD) can only contain letters.' 
    };
  }

  // Check against common TLDs list (more lenient - allow unknown TLDs but validate format)
  // We'll be lenient and allow any valid format, but we can add stricter checking if needed
  // For now, if it passes format validation, we allow it (new TLDs are added frequently)
