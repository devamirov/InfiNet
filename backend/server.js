...
db.serialize(() => {
    ...
    // Note: SQLite doesn't support adding UNIQUE constraint directly in ALTER TABLE
    ...
    db.run(`ALTER TABLE users ADD COLUMN google_id TEXT`, (err) => {
        ...
        if (err && !err.message.includes('duplicate column')) {
            ...
            // Create unique index on google_id if column was added successfully
            db.run(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_google_id ON users(google_id) WHERE google_id IS NOT NULL`, (indexErr) => {
                if (indexErr) {
                    console.error('Error creating google_id unique index:', indexErr);
                }
            });
        }
    });
    db.run(`ALTER TABLE users ADD COLUMN google_email TEXT`, (err) => {
        // Ignore error if column already exists
        if (err && !err.message.includes('duplicate column')) {
            console.error('Error adding google_email column:', err);
        }
    });
    
    // Set admin role for the admin user
    db.run(
        `UPDATE users SET role = 'Administrator' WHERE email = 'amirxtet@gmail.com'`,
        (err) => {
            if (err) {
                console.error('Error setting admin role:', err);
            } else {
                console.log('✅ Admin role set for amirxtet@gmail.com');
            }
        }
    );
...
function createOrUpdateGoogleUser(googleId, googleEmail, email) {
    return new Promise((resolve, reject) => {
        // First, try to find user by Google ID
        getUserByGoogleId(googleId).then(existingByGoogleId => {
            if (existingByGoogleId) {
                // Update existing user
                db.run(
                    'UPDATE users SET google_email = ?, email = ?, emailVerified = 1, updatedAt = CURRENT_TIMESTAMP WHERE google_id = ?',
                    [googleEmail, email, googleId],
                    function(err) {
                        if (err) {
                            reject(err);
                        } else {
                            resolve({ ...existingByGoogleId, google_email: googleEmail, email: email, isNew: false });
                        }
                    }
                );
            } else {
                // Try to find by email
                getUserByEmail(email).then(existingByEmail => {
                    if (existingByEmail) {
                        // Update existing user with Google info
                        db.run(
                            'UPDATE users SET google_id = ?, google_email = ?, emailVerified = 1, updatedAt = CURRENT_TIMESTAMP WHERE email = ?',
                            [googleId, googleEmail, email],
                            function(err) {
                                if (err) {
                                    reject(err);
                                } else {
                                    resolve({ ...existingByEmail, google_id: googleId, google_email: googleEmail, isNew: false });
                                }
                            }
                        );
                    } else {
                        ...
                        const userRole = normalizedEmail === 'amirxtet@gmail.com' ? 'Administrator' : 'Member';
                        // Use empty string for password_hash instead of NULL to satisfy NOT NULL constraint
                        db.run(
                            'INSERT INTO users (email, google_id, google_email, name, role, emailVerified, password_hash) VALUES (?, ?, ?, ?, ?, 1, ?)',
                            [normalizedEmail, googleId, googleEmail, googleEmail.split('@')[0] || 'User', userRole, ''],
                            function(err) {
                                if (err) {
                                    reject(err);
                                } else {
                                    db.get('SELECT * FROM users WHERE id = ?', [this.lastID], (err, row) => {
                                        if (err) reject(err);
                                        else resolve({ ...row, isNew: true, created_at: new Date().toISOString() });
                                    });
                                }
                            }
                        );
...
// These are provided as defaults but should be moved to environment variables for security
const groqApiKeys = [
  process.env.GROQ_API_KEY_MAIN || process.env.WEBSITE_CHATBOT_GROQ_MAIN || '',     // Main Groq key
  process.env.GROQ_API_KEY_FALLBACK || process.env.WEBSITE_CHATBOT_FALLBACK_GROQ || ''  // Fallback Groq key
].filter(key => key !== null);
                            ...
                            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" style="max-width: 600px; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); overflow: hidden;">
                                <tr>
                                    <td style="background: linear-gradient(135deg, #72FF13 0%, #72FF13 30%, #00F0FF 30%, #00F0FF 100%); padding: 40px 30px; text-align: center;">
                                        ...
                                        <p style="margin: 12px 0 0 0; color: #000000; font-size: 16px; opacity: 0.95;">Your consultation with Mr. Amir Khteit has been successfully scheduled.</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding: 40px 30px;">
                                        ...
                                        <div style="background: linear-gradient(135deg, #E0F7FF 0%, #E0F7FF 100%); padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #060097;">
                                            ...
                                            <p style="margin: 8px 0; color: #000000; font-size: 14px; line-height: 1.6;"><strong>👨‍ Meeting with:</strong> Mr. Amir Khteit</p>
                                        </div>
    ...
    try {
        ...
        `, [ticketId, projectId, subject, message, userId, user.name || user.email], function(err) {
            ...
            `, [
                ...
                ticketId,
                'tickets',
                'amirxtet@gmail.com', // Admin email
                0, // unread
...
app.post('/api/test-verification-email', async (req, res) => {
    try {
        ...
        const testEmail = email || 'amirxtet@gmail.com';
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (normalizedEmail === 'amirxtet@gmail.com' && user.role !== 'Administrator') {
        ...
        if (!user.role) {
            user.role = normalizedEmail === 'amirxtet@gmail.com' ? 'Administrator' : 'Member';
        }
        ...
        const userData = {
            ...
            role: normalizedEmail === 'amirxtet@gmail.com' ? 'Administrator' : (user.role || 'Member'),
...
// This endpoint accepts an access token from the mobile app, verifies it with Google,
// and creates/authenticates the user in the main backend (NO Telegram notifications for mobile)
app.post('/api/auth/google/mobile', async (req, res) => {
    try {
            ...
            https.get(userInfoUrl, (response) => {
                ...
                response.on('end', () => {
                    if (response.statusCode === 200) {
                        ...
                        reject(new Error(`Google API error: ${response.statusCode}`));
                    }
                });
            }).on('error', (error) => {
                reject(error);
        ...
        // Create or update user (same logic as hosting backend, but in main backend)
        const user = await createOrUpdateGoogleUser(googleId, googleEmail, email);
        
        // NOTE: NO Telegram notifications for mobile app signups
        // Telegram notifications are only for website hosting signups

        // Generate JWT token
        const token = generateToken(user);

        // Return user data in the format expected by mobile app
        const userData = {
            id: user.id,
            email: user.email || user.google_email,
            name: user.name || googleEmail.split('@')[0] || 'User',
        ...
        if (error.message && error.message.includes('Google API error')) {
...
// Proxy POST /api/convert to file-converter-service (mobile app File Converter tool)
const FILE_CONVERTER_URL = process.env.FILE_CONVERTER_URL || 'http://localhost:3001';
app.post('/api/convert', async (req, res) => {
  try {
    const url = `${FILE_CONVERTER_URL}/api/convert`;
    ...
    console.error('File converter proxy error:', err?.message || err);
    res.status(502).json({ error: 'File conversion service unavailable. Please try again later.' });
  }
});
